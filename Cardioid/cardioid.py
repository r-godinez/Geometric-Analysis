...
def plot_cardioid(a, color):
    """
    Plots a cardioid with the polar equation: r = a * (1 + cos(theta))

    Parameters:
    a (float): The parameter that controls the size of the cardioid.
    color (str): The color of the cardioid.
    ...
    ax.clear()
    ax.plot(x, y, color=color, label=f"Cardioid (a={a})")
    ax.set_aspect('equal', 'box')
    ax.grid(True)
    ax.set_title(f'Cardioid with a={a}')
    canvas.draw()
...
def update_plot():
    ...
    plot_cardioid(a, selected_color[0])

def choose_color():
    """
    Opens a color chooser dialog and updates the color of the cardioid.
    """
